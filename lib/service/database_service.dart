...
class DatabaseService {
  ...
  Future createGroup(
    ...
    DocumentReference groupDocumentReference = await groupCollection.add({
      ...
      "recentMessageSender": "",
      "isPrivate": isPrivate,
      // For production, never store plaintext passwords. Use secure hashing.
      "password": password,
    });
    // update the members
  ...
  Future<bool> isGroupPrivate(String groupId) async {
    ...
    return (data["isPrivate"] ?? false) as bool;
  }

  Future<bool> verifyGroupPassword(String groupId, String password) async {
    DocumentSnapshot snapshot = await groupCollection.doc(groupId).get();
    if (!snapshot.exists) return false;
    final data = snapshot.data() as Map<String, dynamic>;
